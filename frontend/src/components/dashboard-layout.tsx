...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  const navItems = [
    ...
    { icon: ShieldAlert, label: "Risk Center", href: "#risk-center" },
  ];
    ...
    <div className="flex h-screen bg-[#020617] text-slate-200 overflow-hidden font-sans">
      ...
      <aside className="w-64 border-r border-slate-800 bg-[#020617] flex flex-col">
        ...
        <div className="p-4 border-t border-slate-800 mt-auto">
          <div className="flex items-center gap-3 px-3 py-2 rounded-lg cursor-not-allowed opacity-60">
            <Settings className="w-5 h-5 text-slate-400" />
            <span className="text-sm font-medium text-slate-500">Settings</span>
            <span className="ml-auto text-[10px] text-slate-600 font-semibold uppercase tracking-wider">Coming Soon</span>
          </div>
        </div>
      </aside>
